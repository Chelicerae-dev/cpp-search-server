...
void SearchServer::RemoveDocument(ExecutionPolicy&& policy, int document_id) {
    ...
    if(typeid(policy).name() == typeid(std::execution::par).name()) {
        ...
        std::vector<const std::pair<const std::string, double> *> words(documents_by_id_.at(document_id).size());
        ...
        std::for_each(policy, words.begin(), words.end(), [this, document_id, &temp_words](const auto &value) {
            this->word_to_document_freqs_.at(value->first).erase(document_id);
            if (this->word_to_document_freqs_.at(value->first).size() == 0) {
        ...
        if (!temp_words.empty()) {
            std::for_each(policy, temp_words.begin(), temp_words.end(), [this](const std::string &word) {
...
std::tuple<std::vector<std::string>, DocumentStatus> SearchServer::MatchDocument(ExecutionPolicy&& policy, const std::string& raw_query, int document_id) const {
    ...
    if(typeid(policy).name() == typeid(std::execution::par).name()) {
        ...
        if(std::any_of(policy, minus_words.begin(), minus_words.end(), [this, document_id](const std::string& word){
        ...
        auto last = std::copy_if(policy, plus_words.begin(), plus_words.end(), matched_words.begin(), [document_id, this](const std::string& word){
            return word_to_document_freqs_.count(word) != 0 && word_to_document_freqs_.at(word).count(document_id) != 0;
        });
